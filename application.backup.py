...
from helpers import apology, login_required, lookup, usd, transactions
...
def index():
    ...
    # Get transaction histories for buy and sell
    bStocks = transactions("B")
    sStocks = transactions("S")
    fStocks = list()
    fValue = 0

    # Iterate through bought stocks
    for brow in bStocks:
        ...
        symbol = db.execute("SELECT symbol FROM symbols WHERE id = :symbolid", symbolid=brow[0])
        ...
        final_shares = (name_symbol, shares, usd(quote["price"]), usd(value))
        fStocks.append(final_shares)


    return render_template("index.html", username=userdata[0]["username"], cash=usd(userdata[0]["cash"]), \
                           stocks=fStocks, fValue=usd(fValue), total=usd(userdata[0]["cash"] + fValue))
...
def sell():
    ...
    if request.method == "POST":
        ...
        q = db.execute("SELECT symbols FROM symbols JOIN transactions ON symbols.id = transactions.symbol_id WHERE u")
